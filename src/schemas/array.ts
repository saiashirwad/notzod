...
export class ArraySchema<T> extends Schema<T[]> {
	...
	compile(): void {
		...
		if (this.options.minLength !== undefined) {
			...
			checks.push(`  return { success: false, error: { path, message: "Array must have at least ${this.options.minLength} elements", data: value } };`)
			checks.push('}')
		}
		...
		if (this.options.maxLength !== undefined) {
			...
			checks.push(`  return { success: false, error: { path, message: "Array must have at most ${this.options.maxLength} elements", data: value } };`)
			checks.push('}')
		}
